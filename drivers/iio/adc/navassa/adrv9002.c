...
{
	struct adrv9002_rf_phy *phy = iio_priv(indio_dev);
	adi_adrv9001_RxGainControlMode_e val;
	...
	switch (mode) {
	case 0:
		val = ADI_ADRV9001_RX_GAIN_CONTROL_MODE_SPI;
		break;
	case 1:
		val =  ADI_ADRV9001_RX_GAIN_CONTROL_MODE_PIN;
		break;
	case 2:
		val = ADI_ADRV9001_RX_GAIN_CONTROL_MODE_AUTO;
		break;
	default:
		return -EINVAL;
	}
...
static const u32 rx_track_calls[] = {
	...
	[RX_BBDC] = ADI_ADRV9001_TRACKING_CAL_RX_BBDC,
				    ...
				    char *buf)
{
	...
	case TX_RF_BANDWIDTH:
		tx_profile = &phy->curr_profile->tx.txProfile[chan->channel];
		val = tx_profile->primarySigBandwidth_Hz;
		return sprintf(buf, "%d\n", val);
	default:
		return -EINVAL;
	}
...
static const struct iio_chan_spec_ext_info adrv9002_phy_rx_ext_info[] = {
	...
	_ADRV9002_EXT_RX_INFO("bbdc_rejection_tracking_en", RX_BBDC),
...
rf_enable:
		ret = adi_adrv9001_Radio_Channel_ToRfEnabled(adrv9001_dev, ADI_RX,
							     rx->channel.number);
		if (ret)
			return adrv9002_dev_err(phy);
	}
...
rf_enable:
		ret = adi_adrv9001_Radio_Channel_ToRfEnabled(phy->adrv9001, ADI_TX,
							     tx->channel.number);
		if (ret)
			return adrv9002_dev_err(phy);
	}
			  ...
			  adi_adrv9001_Init_t *adrv9002_init)
{
	...
	u8 channel_mask = 0;
	int ret;
	struct adi_adrv9001_RxChannelCfg *adrv9001_rx_chann_cfg;
	adi_adrv9001_gpMaskArray_t gp_mask;

	phy->curr_profile = adrv9002_init;
	adrv9001_rx_chann_cfg = phy->curr_profile->rx.rxChannelCfg;
	...
	ret = adrv9001_rx_path_config(phy);
	if (ret)
		return ret;

	ret = adrv9002_tx_path_config(phy);
	if (ret)
		return ret;
	...
	clk_set_rate(phy->clks[RX1_SAMPL_CLK],
		     adrv9001_rx_chann_cfg[0].profile.rxOutputRate_Hz);
	clk_set_rate(phy->clks[RX2_SAMPL_CLK],
		     adrv9001_rx_chann_cfg[1].profile.rxOutputRate_Hz);
	clk_set_rate(phy->clks[TX1_SAMPL_CLK],
		     phy->curr_profile->tx.txProfile[0].txInputRate_Hz);
				       ...
				       struct adrv9002_rx_chan *rx)
{
	...
	ret = OF_ADRV9002_PINCTL("adi,increment-step-size", 1, 1, 72,
				 rx->pin_cfg->incrementStepSize, false);
	if (ret)
		goto of_pinctrl_put;

	ret = OF_ADRV9002_PINCTL("adi,decrement-step-size", 1, 1, 72,
...
#define ADRV9002_OF_AGC_PIN(key, min, max, val)	\
	...
	/* feedback pins*/
	ret = ADRV9002_OF_AGC_PIN("adi,agc-power-feedback_low-thres_gain-change",
				  ADI_ADRV9001_GPIO_PIN_CRUMB_01_00,
				  ADI_ADRV9001_GPIO_PIN_CRUMB_15_14,
